...
def load_tf_weights(inputbase, config):
    ...
    try:
        ...
        param_names = [key for key in sorted(
            tensor_dict) if "adam" not in key and "global_step" not in key and "pooler" not in key]
        ...
        for pn in param_names:
            ...
            shape_str = "{} ".format(len(shape)) + \
                " ".join([str(d) for d in shape])
            weights_dict[outname] = trt.Weights(flat_tensor)

            TRT_LOGGER.log(TRT_LOGGER.VERBOSE, "Original name: {:}, TensorRT name: {:}, shape: {:}".format(
            ...
            if pos != -1:
                ...
                if config.use_int8 and getattr(config, 'interleaved', False):
                    Wall = np.ascontiguousarray(Wall.reshape(
                        (3, N, H, N, H)), dtype=np.float32)
                    Ball = np.ascontiguousarray(
                        Ball.reshape((3, N, H)), dtype=np.float32)
                else:
                    Wall = np.ascontiguousarray(Wall.reshape(
                        (3, N, H, N, H)).transpose((1, 0, 2, 3, 4)), dtype=np.float32)
                    Ball = np.ascontiguousarray(Ball.reshape(
                        (3, N, H)).transpose((1, 0, 2)), dtype=np.float32)

                additional_dict[prefix + WQKV] = trt.Weights(Wall)
                additional_dict[prefix + BQKV] = trt.Weights(Ball)

                additional_dict[prefix + WQKV +
                                "_notrans"] = trt.Weights(Wall.T)
...
def onnx_to_trt_name(onnx_name):
    ...
    if toks[0] in ('encoder', 'embeddings'):
        toks = ['bert'] + toks
    ...
    for outname, tensor in tensor_dict.items():
        if outname.find("_amax") != -1:
            ...
            if config.use_int8 and getattr(config, 'interleaved', False):
                ...
                Wqkv = np.ascontiguousarray(Wqkv.reshape(
...
def load_onnx_weights_and_quant(path, config):
    ...
    tensor_dict = dict((onnx_to_trt_name(w.name), np.frombuffer(w.raw_data, np.int8).reshape(w.dims))
                       ...
                       (onnx_to_trt_name(w.name), np.frombuffer(
                           w.raw_data, np.float32).reshape(w.dims))
                       for w in weights[1:])
    ...
    for name, tensor in tensor_dict.items():
        ...
        if name.endswith('self_qkv_kernel'):
            tensor = np.ascontiguousarray(
                tensor.reshape((3, N, H, N, H))).astype(np.float32)
            weight_dict[name] = trt.Weights(tensor)
        elif name.endswith('self_qkv_bias'):
            tensor = np.ascontiguousarray(
                tensor.reshape((3, N, H))).astype(np.float32)
            weight_dict[name] = trt.Weights(tensor)
            ...
            if name.endswith('_qkv_input_amax'):
                weight_dict[name.replace(
                    '_qkv_input_amax', '_query_input_amax')] = tensor
                weight_dict[name.replace(
                    '_qkv_input_amax', '_key_input_amax')] = tensor
                weight_dict[name.replace(
                    '_qkv_input_amax', '_value_input_amax')] = tensor
        else:
            flat_tensor = np.ascontiguousarray(
