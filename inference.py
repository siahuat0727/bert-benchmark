...
if __name__ == '__main__':
    ...
    with open(args.engine, 'rb') as f, trt.Runtime(TRT_LOGGER) as runtime, \
        ...
        if selected_profile == -1:
            raise RuntimeError(
                "Could not find any profile that can run batch size {}.".format(args.batch_size))
        ...
        def inference(input_ids_batch):
            ...
            # Run inference
            context.execute_async_v2(bindings=[0 for i in range(binding_idx_offset)] + [int(
                d_inp) for d_inp in d_inputs] + [int(d_output)], stream_handle=stream.handle)
